...
fetchPopularMovies(14).then(response => {
  ...
  const markup = createPaginationMarkupBasedOnScreenSize({ screenWidth, currentPage, totalPages });
...
//===================== Choose a new currentPage based on the user's selection  ===========================//
  function setTargetPage(element, currentPage) {
    ...
    if (
      element.closest('button').classList.contains('arrow-to-end-button-js')
    ) {
      return currentPage + 1;
    }
